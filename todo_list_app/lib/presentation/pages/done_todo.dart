...
class DoneTodosPage extends StatefulWidget {
  ...
  @override
  State<DoneTodosPage> createState() => _DoneTodosPageState();
}
...
class _DoneTodosPageState extends State<DoneTodosPage> {
  ...
  Widget build(BuildContext context) {
    final doneTodos = widget.store.todoList.where((element) => element.isDone).toList();

    if (doneTodos.isNotEmpty) {
      return ListView.builder(
        itemCount: doneTodos.length,
        itemBuilder: (_, index) => _buildListItem(doneTodos[index], context),
      );
    } else {
